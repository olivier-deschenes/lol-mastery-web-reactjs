...
import type { ParticipantType, GoodCachedInfoSchema } from "@/api/match/types";
import {
...
export class DDragonService {
  ...
  static async loadMetadata() {
    ...
    const getProfileIconUrl = (id: MasteryIDType["profile_icon_id"]) => {
      return data.urls.profil_icon.replace("{id}", String(id));
    };

    const getChampionIconUrl = (
      id: MasteryType["champion"]["image"]["full"]
    ) => {
      return data.urls.champion_image.replace("{id}", String(id));
    };

    const getChampionIconUrlFromParticipant = (p: ParticipantType) => {
    ...
    const getSummonerSpellIconUrl = (id: ParticipantType["summoner1Id"]) => {
    ...
    return {
      ...data,
      getQueueName: (queueId: GoodCachedInfoSchema["data"]["gameId"]) => {
        switch (queueId) {
          case 420:
